...
:class:`ContainerInfo` lives at ``client/podman_inspector.py``.

Annotation semantics are caller-owned.  This module doesn't know
about ``ai.terok.*`` keys; whoever pulls a :class:`ContainerInfo`
...
#: Sentinel used as the default value for :attr:`ContainerInfo.annotations`
...
class ContainerInfo:
    ...
    would poison future lookups.  Build with :class:`types.MappingProxyType`
