...
from *container id* to :class:`ContainerInfo` is therefore expressed
here as a pure :class:`ContainerInspector` protocol; the concrete
backend that knows how to talk to a specific runtime lives in
terok-sandbox, where runtime selection is owned.

:class:`NullInspector` ships as a safe default: deployments without
...
class ContainerInspector(Protocol):
    """Callable that maps a container id to a :class:`ContainerInfo`.
    ...
    container / malformed metadata returns an empty :class:`ContainerInfo`
    ...
    def __call__(self, container_id: str) -> ContainerInfo:
        """Return the best-effort :class:`ContainerInfo` for *container_id*."""
        ...


class NullInspector:
    """Always-empty :class:`ContainerInspector` — the graceful-degradation default.
    ...
    def __call__(self, _container_id: str) -> ContainerInfo:
        """Return the universal empty :class:`ContainerInfo`."""
