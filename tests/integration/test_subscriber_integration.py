...
class MockShield1(ServiceInterface):
    """Mock implementation of ``org.terok.Shield1`` — the unified hub interface.
    ...
    def __init__(self) -> None:
        """Initialise with empty verdict log and signal-arg slot."""
        super().__init__(SHIELD_INTERFACE_NAME)
        self._verdict_log: list[tuple[str, str, str, str]] = []
        self._sig_args: list = []

    @signal(name="ConnectionBlocked")
    def connection_blocked(self) -> "sssuus":  # noqa: F722
        """Emit a ConnectionBlocked signal."""
        return self._sig_args

    def emit_connection_blocked(
        self, container: str, request_id: str, dest: str, port: int, proto: int, domain: str
    ) -> None:
        """Convenience emitter for ConnectionBlocked."""
        self._sig_args = [container, request_id, dest, port, proto, domain]
        self.connection_blocked()

    @method(name="Verdict")
    def verdict(  # noqa: F821
        self, container: "s", request_id: "s", dest: "s", action: "s"
    ) -> "b":
        """Record a Verdict call and return success."""
        self._verdict_log.append((container, request_id, dest, action))
        return True

    @signal(name="VerdictApplied")
    def verdict_applied(self) -> "sssb":  # noqa: F722
        """Emit a VerdictApplied signal."""
        return self._sig_args

    def emit_verdict_applied(self, container: str, request_id: str, action: str, ok: bool) -> None:
        """Convenience emitter for VerdictApplied."""
        self._sig_args = [container, request_id, action, ok]
        self.verdict_applied()


class MockClearance1(ServiceInterface):
    """Mock implementation of ``org.terok.Clearance1`` — legacy interface unchanged."""

    def __init__(self) -> None:
        """Initialise with empty resolve log and signal-arg slot."""
        super().__init__(CLEARANCE_INTERFACE_NAME)
        self._resolve_log: list[tuple[str, str]] = []
        self._sig_args: list = []

    @signal(name="RequestReceived")
    def request_received(self) -> "ssssqs":  # noqa: F722
...
async def shield_service(dbusmock_session) -> AsyncIterator[MockShield1]:
    """Export MockShield1 claiming the unified ``org.terok.Shield1`` bus name."""
...
async def clearance_service(dbusmock_session) -> AsyncIterator[MockClearance1]:
    """Export MockClearance1 on the private test bus."""
    bus = await MessageBus().connect()
    svc = MockClearance1()
...
class TestShieldSubscriberIntegration:
    ...
    async def test_connection_blocked_creates_notification(
        ...
        """ConnectionBlocked triggers notify with accept/deny action buttons."""
        ...
        shield_service.emit_connection_blocked(CONTAINER, "req-1", DEST_IP, 443, 6, DOMAIN)
        await asyncio.sleep(0.2)

        mock_notifier.notify.assert_awaited_once()
        call = mock_notifier.notify.call_args
        assert f"{DOMAIN}:443" in call[0][0]
        assert ("allow", "Allow") in call.kwargs["actions"]
    ...
    async def test_action_routes_verdict_to_hub(
        ...
        shield_service.emit_connection_blocked(CONTAINER, "req-2", DEST_IP, 443, 6, DOMAIN_ALT)
        await asyncio.sleep(0.2)

        assert mock_notifier.on_action.call_args[0][0] == 1
    ...
    async def test_verdict_applied_updates_notification(
        ...
        """VerdictApplied(container, request_id, action, ok) replaces the in-place notification."""
        mock_notifier = AsyncMock()
        mock_notifier.notify = AsyncMock(return_value=1)
        mock_notifier.on_action = AsyncMock()
        ...
        shield_service.emit_connection_blocked(CONTAINER, "req-3", DEST_IP, 443, 6, DOMAIN)
        await asyncio.sleep(0.2)

        mock_notifier.notify.reset_mock()
        shield_service.emit_verdict_applied(CONTAINER, "req-3", "allow", True)
        await asyncio.sleep(0.2)
...
class TestClearanceSubscriberIntegration:
    ...
    async def test_request_received_creates_notification(
        ...
        """RequestReceived triggers notify with accept/deny action buttons."""
        mock_notifier = AsyncMock()
        mock_notifier.notify = AsyncMock(return_value=1)
        mock_notifier.on_action = AsyncMock()
    ...
    async def test_action_routes_resolve_to_service(
        ...
        """Operator action routes Resolve(request_id, action) to the clearance service."""
        mock_notifier = AsyncMock()
        mock_notifier.notify = AsyncMock(return_value=1)
        mock_notifier.on_action = AsyncMock()
    ...
    async def test_request_resolved_updates_notification(
        ...
        """RequestResolved updates the notification in place with the resolution outcome."""
        mock_notifier = AsyncMock()
        mock_notifier.notify = AsyncMock(return_value=1)
        mock_notifier.on_action = AsyncMock()
